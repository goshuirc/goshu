...
def auto_unicode_decode( s ):
    ...
    bom2 = s[:2]
    bom4 = s[:4]
    a, b, c, d = list(map(ord, s[:4]))  # values of first four bytes
...
def isnumbertype( obj ):
    ...
    return isinstance(obj, (int,int,float)) \
...
class JSON(object):
    ...
    def __init__(self, strict=False, compactly=True, escape_unicode=False):
        ...
        self._asciiencodable = [32 <= c < 128 and chr(c) not in self._rev_escapes
    ...
    def decode_string(self, s, i=0, imax=None):
        ...
        converted_chunks = []
        for chunk in chunks:
            converted_chunks.append(str(chunk))
        s = ''.join( converted_chunks )
        return s, i
    ...
    def encode_helper(self, chunklist, obj, nest_level):
        ...
        elif isinstance(obj, (int,int,float,complex)) or \
    ...
    def encode_equivalent( self, obj, nest_level=0 ):
        ...
        """
        if hasattr(obj, 'json_equivalent') \
               and hasattr(getattr(obj,'json_equivalent'), '__call__'):
    ...
    if encoding is not None:
        ...
        if cdk:
            ...
            if encoding.startswith('utf-32') or encoding.startswith('utf32') \
                ...
                encoding = 'utf-16'
                if not escape_unicode and not hasattr(escape_unicode, '__call__'):
