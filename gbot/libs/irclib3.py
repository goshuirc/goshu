...
#
# danneh <danneh@danneh.net>
#
# $Id: irclib3.py,v 3.00 <datetime> danneh Exp $

"""irclib3 -- Internet Relay Chat (IRC) protocol client library, for Py3.
...
class ServerConnection(Connection):
	...
	def process_data(self):
		...
		try:
			if self.ssl:
				new_data = self.ssl.read(2**14).decode(str_encoding)
			else:
				new_data = self.socket.recv(2**14).decode(str_encoding) #handle fail
		except socket.error:#>>(x):
			# The server hung up.
	...
	def send_raw(self, string):
		...
		try:
			if self.ssl:
				...
				self.socket.send(bytes(string, str_encoding) + b"\r\n")
			if DEBUG:
...
class DCCConnection(Connection):
	...
	def process_data(self):
		...
		try:
			new_data = self.socket.recv(2**14).decode(str_encoding)
		except socket.error(x):
			# The server hung up.
			self.disconnect("Connection reset by peer")
			return
	...
	def privmsg(self, string):
		...
		try:
			self.socket.send(bytes(string, str_encoding))
			if self.dcctype == "chat":
				self.socket.send(b"\n")
			if DEBUG:
				print("TO PEER: %s\n" % string)
...
_ircstring_translation = bytes.maketrans(bytes(string.ascii_uppercase, str_encoding) + b"[]\\^",
										 bytes(string.ascii_lowercase, str_encoding) + b"{}|~")
