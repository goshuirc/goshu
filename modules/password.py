...
class Password(Module):
	...
	def change_password(self, line, connection, event):
		if line == None and connection == None and event == None:
			text_commands_info = {
				'password' : [10, 10, 'change bot password'],
			}
		...
		if self.gbot.level(event.source()) >= self.gbot.text_commands_info['password'][1]:
			...
			output = 'Unauthorised \'' + self.gbot.prefix + 'password\' '
